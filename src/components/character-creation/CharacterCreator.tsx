...
const resourceLabels = { stamina: "סיבולת", mana: "מאנה", focus: "מיקוד", faith: "אמונה", rage: "זעם" } as const;
...
export function CharacterCreator() {
  ...
  useEffect(() => {
    try {
      ...
      if (raw) queueMicrotask(() => setDraft({ ...defaultDraft, ...JSON.parse(raw) as CharacterDraftInput }));
    } catch {
  ...
  const chooseRace = (raceId: RaceId) => {
    ...
    setDraft((current) => ({ ...current, raceId, portraitKey: selected.portraitKeys[0] }));
  };
        ...
        <RunePanel className="min-h-[34rem] p-4 sm:p-7">
          <AnimatePresence mode="wait">
            <motion.div key={step} initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -12 }} transition={{ duration: 0.22 }}>
              ...
              {step === "class" ? <ClassStep selected={draft.classId} onSelect={(classId) => update("classId", classId)} /> : null}
              ...
              {step === "appearance" ? <AppearanceStep race={race} selected={draft.portraitKey} onSelect={(portraitKey) => update("portraitKey", portraitKey)} /> : null}
              {step === "summary" ? <SummaryStep draft={draft} attributes={finalAttributes} derived={derived} /> : null}
            </motion.div>
          </AnimatePresence>
        </RunePanel>
...
function AppearanceStep({ race, selected, onSelect }: { race: (typeof characterRaces)[number]; selected: string; onSelect: (key: string) => void }) {
  return <div><StepHeading title="פנים למסע" text="בחרו דיוקן שנוצר באותה שפה חזותית של העולם ויופיע בדיאלוגים, בקרב ובחבורה." /><div className="grid grid-cols-2 gap-3 sm:grid-cols-4">{race.portraitKeys.map((key, index) => <button key={key} onClick={() => onSelect(key)} className={`group relative aspect-[3/4] overflow-hidden border-2 bg-[#101318] ${selected === key ? "border-[#f0cf82] shadow-[0_0_26px_rgba(98,198,223,.24)]" : "border-white/10 hover:border-[#c6a15b]/60"}`} aria-pressed={selected === key}><Image src={getAssetPath(key)} alt={`דיוקן ${race.name}, אפשרות ${index + 1}`} fill sizes="(max-width: 640px) 50vw, 25vw" className="object-cover transition-transform duration-300 group-hover:scale-[1.03]" />{selected === key ? <span className="absolute end-2 top-2 grid size-8 place-items-center rounded-full bg-[#c6a15b] text-black"><Check className="size-5" /></span> : null}</button>)}</div></div>;
}

function SummaryStep({ draft, attributes, derived }: { draft: CharacterDraftInput; attributes: Attributes; derived: ReturnType<typeof deriveStats> }) {
