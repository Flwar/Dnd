...
import { BookOpen, Crown, LogOut, Settings, Shield, Swords, UserRoundPlus, UsersRound } from "lucide-react";
...
export function MainMenuClient({ displayName, characters }: { displayName: string; characters: CharacterSummary[] }) {
  ...
  return (
    <main id="main-content" className="screen-shell flex min-h-dvh items-center px-4 py-8 sm:px-8 lg:px-[7vw]">
      ...
      <motion.section className="relative z-10 w-full max-w-3xl" initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.55 }}>
        <div className="mb-7">
          <button className="mb-2 text-sm text-[#b9ad9c] hover:text-[#f0cf82]" onClick={() => router.push("/profile")}>ברוכים השבים, {displayName}</button>
          <div className="flex items-center gap-3"><Crown className="size-8 text-[#62c6df]" aria-hidden="true" /><h1 className="display-font text-5xl text-[#f0cf82] sm:text-7xl">הכתר המנופץ</h1></div>
        </div>
        ...
        {active ? (
          ...
          >
            ...
            <div className="hidden size-24 overflow-hidden border border-[#c6a15b]/35 bg-black/30 sm:block">
              {/* eslint-disable-next-line @next/next/no-img-element -- generated local portrait */}
              <img src={getAssetPath(active.portraitPath)} alt={`דיוקן של ${active.name}`} className="size-full object-cover transition-transform duration-300 group-hover:scale-105" />
            </div>
          </button>
        ) : null}
