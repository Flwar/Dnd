    ...
    <div className="mx-auto max-w-5xl">
      ...
      <div className="grid gap-5 md:grid-cols-[19rem_1fr]">
        <section className="stone-panel p-5 text-center"><div className="relative mx-auto size-48 overflow-hidden border border-[#c6a15b]/45">
          <Image src={getAssetPath(avatarKey)} alt="דיוקן הפרופיל" fill sizes="192px" className="object-cover" />
        ...
        <div className="space-y-5">
          ...
          <form className="stone-panel p-5" onSubmit={(event) => { event.preventDefault(); setMessage(""); startTransition(async () => { const result = await updateProfileAction({ displayName, avatarKey }); setMessage(result.message); }); }}>
            ...
            <fieldset className="mt-5"><legend className="mb-2 font-bold">סמל שחקן</legend><div className="grid grid-cols-3 gap-2 sm:grid-cols-6">{avatarKeys.map((key) => <button type="button" key={key} className={`relative aspect-square overflow-hidden border ${avatarKey === key ? "border-[#f0cf82] ring-2 ring-[#f0cf82]/20" : "border-white/15"}`} onClick={() => setAvatarKey(key)} aria-pressed={avatarKey === key}>
              <Image src={getAssetPath(key)} alt="אפשרות לדיוקן פרופיל" fill sizes="96px" className="object-cover" />
            </button>)}</div></fieldset>
